...
let invoke ?(creds = Credentials.try_load ()) ?payload ~func_name () =
  ...
  let$ creds =
    creds
    |> Option.to_result ~none:(Error.RequestError "Failed to load credentials")
  in
  let$ region =
  ...
  let$ req =
    ...
    |> Option.map (Request.with_auth_header ~datetime:now ~access_id:creds.access_id ~access_secret:creds.access_secret ~region ~service)
  ...
  let& response =
    Request.perform req
    |> Lwt.map (Result.map_error Error.curlError)
  in
  return (Ok response)
;;
